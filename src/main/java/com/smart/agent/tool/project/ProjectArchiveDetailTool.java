...
@Component
public class ProjectArchiveDetailTool implements AgentTool<ProjectArchiveDetailInput, ProjectArchiveDetailResult> {
    private final ProjectBusinessClient projectBusinessClient;

    public ProjectArchiveDetailTool(ProjectBusinessClient projectBusinessClient) {
        this.projectBusinessClient = projectBusinessClient;
    }

    @Override public String key() { return "project.getArchiveDetail"; }
    @Override public Class<ProjectArchiveDetailInput> inputType() { return ProjectArchiveDetailInput.class; }
    @Override public String requiredPermission() { return "menu:project"; }
    @Override public ToolRisk risk() { return ToolRisk.L1; }

    @Override
    public String description() {
        return "查询当前用户有权访问的单个项目完整项目档案，可使用项目ID、项目编号或项目名称";
    }
    ...
    @Override
    public ProjectArchiveDetailResult execute(ProjectArchiveDetailInput input, ToolContext context) {
        String identifier = input.identifier();
        if (context.canAccessProject(identifier)) {
            return projectBusinessClient.getArchiveDetail(context, identifier);
        }
        int page = 1;
        do {
            ...
            Optional<AccessibleProjectItem> match = projects.items().stream()
                    ...
                    .findFirst();
            if (match.isPresent()) {
                return projectBusinessClient.getArchiveDetail(context, match.get().projectId());
            }
    ...
    @Override
    public Optional<String> projectId(ProjectArchiveDetailInput input) {
