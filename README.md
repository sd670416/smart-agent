...
智能助手提供只读工具 `project.getArchiveDetail`。用户按项目 ID、项目编号或项目名称询问“项目详情”或“完整档案”时，工具通过一次内部聚合调用返回当前用户可见的基础信息、成员、合同、资金、施工、成本、分包、招投标、保证金和审批流程。

工具沿用原系统权限：目标项目必须在可信项目范围内；每个档案分区受对应菜单权限控制；分区内明细继续由 MyBatis 数据权限过滤。查询不额外要求按钮权限，无权限分区不会返回数量或明细。内部请求使用短 HMAC 签名身份头，不转发浏览器 Bearer Token。

聚合结果超过 64KB 时不会截断或发送给模型，而是提示用户指定要查看的部分。开发模式可显示原始工具调试数据，但最终聊天正文不会展示租户 ID、项目 ID、删除标记、版本号等系统字段。
